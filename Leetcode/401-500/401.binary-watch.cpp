...
class Solution {
public:
    void getPossibleNumber(int cur_sum, int cur_weight, int left_number, int max_weight, vector<int>& v) {
        if (left_number == 0) {
            v.push_back(cur_sum);
            return;
        }
        ...
        getPossibleNumber(cur_sum + cur_weight, cur_weight * 2, left_number - 1, max_weight, v);
